...
// Used by both flows: the order-status card, and step 1 of the return
// flow (it needs the order's items before it can show the item picker).
export async function GET(req: NextRequest) {
  ...
  if (orderResult.rows.length === 0) {
    return NextResponse.json(
      { error: `We couldn't find an order matching "${orderId}". Double check the order number and try again.` },
      { status: 404 }
    );
  }
